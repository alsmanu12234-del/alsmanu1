...
async function _fetchChannelNameFromWeb(inviteCode) {
  try {
    ...
    if (ogMatch) {
      const name = ogMatch[1].replace(/ \| WhatsApp Channel.*$/i, "").replace(/ \| .*$/i, "").trim();
      if (name && name.length > 0) return name;
    }
    // fallback: <title>
    const titleMatch = html.match(/<title>([^<]+)<\/title>/i);
    if (titleMatch) {
      const name = titleMatch[1].replace(/ \| WhatsApp.*$/i, "").replace(/ \| .*$/i, "").trim();
      if (name && name.length > 0 && name !== "WhatsApp") return name;
    }
    return null;
  } catch {
    return null;
...
export async function handleForwardCallback(query) {
  ...
  if (data.startsWith(srcPfxCh) || data.startsWith(srcPfxGr)) {
    ...
    if (!items.length) {
      ...
      if (!items.length && !isCh && _sock) {
        items = await fetchGroups().catch(() => []);
      ...
      try {
        ...
        if (_sock?.newsletterMetadata) {
          try {
            ...
            if (meta?.id) { metaId = meta.id; metaName = meta.name || null; }
          } catch {}
        }
